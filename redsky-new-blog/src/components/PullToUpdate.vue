...
// labels of pull down
const LABELS = ['111111', '닫히는 중', '아래로 당겨 데이터 새로고침', '로딩중...'];
...
export default {
  ...
  computed: {
    ...
    customLabels() {
      const errorLabel = this.config.errorLabel !== undefined ? this.config.errorLabel : LABELS[0];
      const startLabel = this.config.startLabel !== undefined ? this.config.startLabel : LABELS[1];
      const readyLaebl = this.config.readyLabel !== undefined ? this.config.readyLabel : LABELS[2];
      const loadingLabel = this.config.loadingLabel !== undefined ? this.config.loadingLabel : LABELS[3];
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      const icon = pullDownHeader.querySelector('.pull-down-content--icon');
       ...
       */
      const resetPullDown = (pullDown, withAnimation) => {
        if (withAnimation) {
